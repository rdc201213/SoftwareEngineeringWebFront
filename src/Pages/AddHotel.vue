<template>
  <div>
    ...
    <el-button
      type="primary"
      style="margin-left: 10px; float: right; margin-top: 40px"
      icon=""
      @click.native="addressForm()"
      >添加</el-button
    >
    <el-card class="box-card">
      <div class="text item">酒店名称：{{ hotelAddress.hotelName }}</div>
      <div class="text item">所在省份：{{ hotelAddress.province }}</div>
      <div class="text item">所在城市：{{ hotelAddress.city }}</div>
      <div class="text item">所在区：{{ hotelAddress.district }}</div>
      <div class="text item">简介：{{ hotelAddress.introduction }}</div>
      <div class="text item">特殊服务：{{ hotelAddress.specialService }}</div>
    </el-card>
    ...
    <el-dialog title="填写地址信息" :visible.sync="addressVisible" width="30%">
      <el-form ref="address" :model="AddressInfo" label-width="150px">
        ...
        <el-form-item label="简介">
          <el-input
            v-model="AddressInfo.introduction"
            style="width: 250px"
            placeholder=""
          ></el-input>
        </el-form-item>
        ...
        <el-form-item label="特殊服务">
          <el-input
            v-model="AddressInfo.specialService"
            style="width: 250px"
            placeholder=""
          ></el-input>
        </el-form-item>
    ...
    <el-dialog title="填写房源信息" :visible.sync="addHotelVisible" width="30%">
      <el-form ref="addHotelForm" :model="HotelInfo" label-width="150px">
        ...
        <el-form-item label="房间特殊服务">
          <el-input
            v-model="HotelInfo.specialService"
            style="width: 250px"
            placeholder=""
          ></el-input>
        </el-form-item>
    ...
    axios
      ...
      .then((response) => {
        if (response.status == 200) {
          console.log(
            "response.data.hotelAddress:" + response.data.hotelAddress
          );
